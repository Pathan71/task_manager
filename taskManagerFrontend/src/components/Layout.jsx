  ...
  if (error) return (
    <div className='min-h-screen bg-gray-50 p-6 flex items-center justify-center'>
      <div className='bg-red-50 text-red-600 p-4 rounded-xl border border-red-100 max-w-md'>
        ...
        <button onClick={fetchTasks} className='mt-4 py-2 px-4 bg-red-100 text-red-700 rounded-lg text-sm font-medium hover:bg-red-100 transition-colors'>
          Try Again
        </button>
      </div>
    </div>
          ...
          <div className='xl:col-span-1 space-y-4 sm:space-y-6'>
            <div className='bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-purple-100'>
              ...
              <div className='grid grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6'>
                <StartCard title='Total Tasks' value={stats.totalCount} icon={<Circle className='w-3.5 h-3.5 sm:w-4 sm:h-4 text-purple-500' />} />
                <StartCard title='Completed' value={stats.completedTasks} icon={<Circle className='w-3.5 h-3.5 sm:w-4 sm:h-4 text-green-500' />} />
                <StartCard title='Pending' value={stats.pendingCount} icon={<Circle className='w-3.5 h-3.5 sm:w-4 sm:h-4 text-fuchsia-500' />} />
                <StartCard title='Completion Rate' value={`${stats.completionPercentage}% `} icon={<Zap className='w-3.5 h-3.5 sm:w-4 sm:h-4 text-purple-500' />} />
              </div>
                ...
                <div className='relative pt-1'>
                  <div className='flex gap-1.5 items-center'>
                    <div className='flex-1 h-2 sm:h-3 bg-purple-100 rounded-full overflow-hidden'>
                      <div className='h-full bg-gradient-to-r from-fuchsia-500 to-purple-600 transition-all duration-300'
