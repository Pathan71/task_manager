...
const TaskModal = ({ isOpen, onClose, taskToEdit, onSave, onLogout }) => {
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      const url = isEdit ? `${API_BASE}/${taskData.id}/tk` : `${API_BASE}/tk`;
  ...
  }, [taskData, today, getHeader, onLogout, onSave, onClose])
    ...
    <div className='fixed inset-0 backdrop-blur-sm bg-black/20 z-50 flex items-center justify-center p-4'>
      <div className='bg-white border-purple-100 rounded-xl max-w-md w-full shadow-lg relative p-6 animate-fadeIn'>
        ...
        <form onSubmit={handleSubmit} className='space-y-4'>
          {error && <div className='text-sm text-red-600 bg-red-100 p-3 rounded-lg border border-red-100'>{error}</div>}
          <div>
            ...
            <div className='flex items-center border border-purple-100 rounded-lg px-3 py-2.5 focus-within:ring-2 focus-within:ring-purple-500 focus-within:border-purple-500 transition-all duration-300'>
              <input type="text" name='title' value={taskData.title}
          ...
          <div className='grid grid-cols-2 gap-4'>
            ...
            <div>
              ...
              <input type="date" name='dueDate' min={today} value={taskData.dueDate}
                onChange={handleChange} className='w-full px-4 py-2.5 border border-purple-100 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 text-sm'
                required />
            </div>
          </div>
          ...
          <div>
            ...
            <div className='flex gap-6'>
              {[{ val: 'Yes', label: 'Completed' }, { val: 'No', label: 'In Progress' }].map(({ val, label }) => (
                <label key={val} className='flex items-center whitespace-nowrap '>
