...
const TaskItem = ({ task, onRefresh, onLogout, showCompeleteCheckbox = true }) => {
  ...
  const handleComplete = async () => {
    ...
    try {
      await axios.put(`${API_BASE}/${task._id}/tk`, { completed: newStatus },
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`${API_BASE}/${task._id}/tk`, { headers: getAuthHeaders() })
      onRefresh?.()
    } catch (err) {
      if (err.response?.stauts === 401) onLogout?.()
    }
  }
  ...
  const handleSave = async (updatedTask) => {
    try {
      ...
      await axios.put(`${API_BASE}/${task._id}/tk`, payload, {
        headers: getAuthHeaders()
      })
      onRefresh?.()
    }
  ...
  return (
    <>
      <div className={`group my-4 p-4 sm:p-5 rounded-xl shadow-sm bg-white border-l-4 hover:shadow-md transition-all duration-300 border border-purple-100 ${borderColor}`}>
        <div className='flex items-start gap-2 sm:gap-3 flex-1 min-w-0'>
          ...
          <div className='flex-1 min-w-0'>
            <div className='flex items-center gap-2 mb-1 flex-wrap'>
              <h3 className={`text-base sm:text-lg font-medium truncate ${isCompleted ?
              ...
              <div className='absolute right-0 mt-1 w-40 sm:w-48 bg-white border border-purple-100 rounded-xl shadow-lg z-10 overflow-hidden animate-fadeIn'>
                {MENU_OPTIONS.map(opt => (
                  <button key={opt.action} onClick={() => handleAction(opt.action)}
                    className='w-full px-3 sm:px-4 py-2 text-left text-xs sm:text-sm hover:bg-purple-50 flex items-center gap-2 transition-all duration-200'>
                    {opt.icon}{opt.label}
                  </button>
                ))}
              </div>
